...
（`*/30 * * * *` で30分おき）。書式上の最短は5分ですが、`*/5` は GitHub 側に
間引かれて発火しなくなりやすいため、15分を既定にしています。

### 自動更新が一度も動かないとき

`schedule` が1件も実行されない場合、まず次の2点を確認してください。

1. **リポジトリが public になっているか。**
   無料プランでは、プライベートリポジトリの `schedule` は実行されません。
   手動実行（`workflow_dispatch`）や push だけは動くので、
   「ワークフローは正常なのに定期実行だけ来ない」という紛らわしい状態になります。

2. **public にしたあとで、既定ブランチに1回 push したか。**
   cron の登録は push のタイミングで同期されます。可視性を変えただけでは
   登録が更新されないことがあるため、public 化のあとに空コミットでもよいので
   `main` へ push してください。

    git commit --allow-empty -m "resync schedule"
    git push

新しい cron が認識されるまで15分〜1時間ほどかかります。それを過ぎても
`gh run list --event=schedule` が空のままなら、`schedule` に頼らず
外部から `workflow_dispatch` を叩く方式に切り替えるのが確実です。
