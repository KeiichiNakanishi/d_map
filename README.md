...
Notion の投票を5分おきに読み取り、決定した行き先を簡易マップ上にピンで表示して
...
.github/workflows/update.yml  5分おきの自動更新
```
...
トークンを安全に扱えないため）。実際の取得は5分おきの GitHub Actions が行い、
ボタンはその結果を取りに行くだけです。
...
地図は次のビルド（最大5分後、または手動実行）で追従します。**push は不要です。**
